...
class UniformReplay(Storage):
    ...
    def compute_valid_indices(self):
        indices = []
        indices.extend(list(range(
            self.history_length - 1, self.pos - self.n_step)))
        indices.extend(list(range(
            self.pos + self.history_length - 1, self.size() - self.n_step)))
    ...
    def sample(self, batch_size=None):
        ...
        while len(sampled_data) < batch_size:
            transition = self.construct_transition(
    ...
    def valid_index(self, index):
        if index - self.history_length + 1 >= 0 and \
                index + self.n_step < self.pos:
            return True
        if index - self.history_length + 1 >= self.pos and \
                index + self.n_step < self.size():
            return True
        return False
    ...
    def construct_transition(self, index):
        ...
        next_state = [self.state[i]
                      for i
                      in range(next_s_start, next_s_end + 1)]
...
class PrioritizedReplay(UniformReplay):
    ...
    def __init__(self,
        ...
        super(PrioritizedReplay, self).__init__(
            memory_size, batch_size, n_step, discount, history_length, keys)
...
class ReplayWrapper(mp.Process):
    ...
    def __init__(self, replay_cls, replay_kwargs, asynchronous=True):
    ...
    def run(self):
        ...
        def set_up_cache():
            ...
            batch_data = [Tensor(x).torch_tensor_for_device()
                          for x in batch_data]
